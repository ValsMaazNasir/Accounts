...
{ 
    public class CashController : BaseController
    {
        ...
        public JsonResult Listener(DTParameters param)
        {
            ...
            {
                dataSource = Database.ChartOfAccounts.Where(o => o.IsDeleted == false && o.Name.ToLower().Contains("cash")).AsQueryable();
            }
            else
            {
                dataSource = Database.ChartOfAccounts.Where(o => o.IsDeleted == false && o.Name.ToLower().Contains("cash") && o.CreatedBy == CurrentUserRecord.ID).AsQueryable();
            }
        ...
        public ActionResult Edit(int? id)
        {
            ...
            {
                return Redirect(ViewBag.WebsiteURL + "cash");
            }
        }
            ...
            try
            {
                if (IsUserLogin())
                {
                    ...
                    if (isAbleToUpdate)
                    {
                        ...
                        AjaxResponse.TargetURL = ViewBag.WebsiteURL + "cash";
                        AjaxResponse.Success = true;
                    }
                }
                else
