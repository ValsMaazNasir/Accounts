...
namespace LiquadCargoManagment.Areas.Accounts.Controllers
{
    public class ChartOfAccountsController: BaseController
    {
        ...
        public void FillDropDown()
        {
            ...
            ViewBag.AccountsHead = Database.AccountsHeads.Where(x => x.IsDeleted == false && x.Status == EnumStatus.Enable).ToList();
            ViewBag.GLTypes = Database.GLTypes.Where(x => x.IsDeleted == false && x.Status == EnumStatus.Enable).ToList();
            ViewBag.SubAccounts = Database.SubAccounts.Where(x => x.IsDeleted == false && x.Status == EnumStatus.Enable).ToList();
        }
